...
<html>
	...
	<body class='extent ${theme} hide-overflow'>
		<!-- nav -->
		<nav>
			<div class='logo-container' style="background-color:white !important;">
			...
			<div id='dashboard-view' class='row'>
				<div class='time-totals'>
					<div class='col l2 m4 s6'>
						<div class='card suite-total-tests' style="background:#00af00 !important">   
							<span class='panel-name' style="color:white !important">${resourceBundle.getString("dashboard.panel.name.totalTests")}</span> 
							<span class='total-tests'> <span class='panel-lead'></span> </span> 
						</div> 
					</div>
					<div class='col l2 m4 s6'>
						<div class='card suite-total-steps' style="background:#f7464a !important"> 
							<span class='panel-name' style="color:white !important">${resourceBundle.getString("dashboard.panel.name.totalSteps")}</span> 
					...
					<div class='col l2 m4 s12'>
						<div class='card suite-total-time-current'> 
							...
							<span class='suite-total-time-current-value panel-lead'>${report.getRunDuration()[0..7]}</span> 
						</div> 
					</div>
					<div class='col l2 m4 s12'>
						<div class='card suite-total-time-overall'> 
							...
							<span class='suite-total-time-overall-value panel-lead'>
							 ${report.getRunDurationOverall()[0..7]}
							</span> 

						</div> 
					</div>
					<div class='col l2 m4 s6 suite-start-time'>
						<div class='card accent green-accent' style="background:#31b131 !important">  
							<span class='panel-name' style="color:white !important">${resourceBundle.getString("dashboard.panel.name.start")}</span> 
					...
					<div class='col l2 m4 s6 suite-end-time'>
						<div class='card accent pink-accent' style="background:#ff6347 !important"> 
							<span class='panel-name' style="color:white !important">${resourceBundle.getString("dashboard.panel.name.end")}</span> 
				...
				<div class='col s12 m12 l4 fh'> 
						<div class='card-panel'> 	
							<div id="columnchart_material" style="width: 400px; height:250px;" align='center'></div>
							<span class='pass-percentage panel-lead' style="display:none"></span> 
					...
					<div class='col s12 m6 l4 fh'> 
						...
						<div class='card-panel '> 
							...
							<div id="steps_view" style="width: 400px; height:250px;" align='center'></div>												
							
							<div class="hide">
								<span class='panel-name'>${resourceBundle.getString("dashboard.panel.name.stepsView")}</span>
							</div> 
							<div class='panel-setting modal-trigger step-status-filter right hide'>
								<a href='#step-status-filter'><i class='mdi-navigation-more-vert text-md'></i></a>
							</div> 
							<div class='chart-box hide'>
								<canvas class='text-centered' id='step-analysis'></canvas>
							</div> 
							<div class="hide">
							<span class='s-warning-count weight-normal' style="display: none"></span>
							<span class='s-error-count weight-normal' style="display: none"></span>
					...
					<span  id="pass-percentage" class='pass-percentage panel-lead' style="display:none"></span> 
							...
							<div class='wrapper'>
								<ul id='test-collection' class='test-collection'>
									<#list report.testList as extentTest>
										...
										<div class='test-head'>
												<span class='category-assigned' style="font-weight: bold;"><#list test.categoryList as category> ${category.name}</#list></span>
										</div>
										...
										<li class='collection-item test displayed active ${test.status} <#if test.nodeList?size != 0>hasChildren</#if>' extentid='${test.id?string}'>
											<div class='test-head'>
											<a href="#">
												<span class='test-name' style="text-decoration: underline;">${test.name} <#if test.internalWarning??><i class='tooltipped mdi-alert-error' data-position='top' data-delay='50' data-tooltip='${test.internalWarning}'></i></#if></span>
												</a>
											...
											<div class='test-body'>
												<div class='test-info'>
													...
													<span title='${resourceBundle.getString("tests.test.info.timeTaken")}' alt='${resourceBundle.getString("tests.test.info.timeTaken")}' class='test-time-taken label blue-grey lighten-3 text-white'><#if test.endedTime??>${test.getRunDuration()[0..7]}</#if></span>
												</div>
														...
														<tbody>
															<#list test.logList as log>
																<tr>
																	...
																	<td class='status ${log.logStatus} myclass' title='${log.logStatus}' alt='${log.logStatus}'>${log.logStatus}</td>
		...
		<script type='text/javascript' src='./extentreports/js/extent.js'></script>
    ...
    function drawChart() {
       var data = google.visualization.arrayToDataTable([
          ['','Pass', 'Fail' ],
          [ 'Details', parseInt( $('#pass-percentage').text(), 10 )/100, (100-parseInt($('#pass-percentage').text(), 10 ))/100]
        ]);
    ...
    1, {
      calc: function (dt, row) {
        return formatPercent.formatValue(dt.getValue(row, 1));
      },
      type: "string",
      role: "annotation"
    },
    ...
    2, {
      ...
      calc: function (dt, row) {
        return formatPercent.formatValue(dt.getValue(row, 2));
      },
      type: "string",
      role: "annotation"
    }
     ...
     var options = {
      ...
      function drawCharts() {
        var data = google.visualization.arrayToDataTable([
          ...
          ['Steps',parseInt($('.s-pass-count').text(),10), parseInt($('.s-fail-count').text(),10),  parseInt($('.s-error-count').text(),10), parseInt($('.s-warning-count').text(),10)]
        ]);
        ...
        var chart = new google.charts.Bar(document.getElementById('steps_view'));
