...
<html>
	...
	<body class='extent ${theme} hide-overflow'>
		...
		<nav>
			...
			<span class='report-name'><#if report.configurationMap??>${report.configurationMap["reportName"]}</#if></span> <span class='report-headline' style="font-size:20px"><#if report.configurationMap??>${report.configurationMap["reportHeadline"]}</#if></span>
			<ul class='right hide-on-med-and-down nav-right'>
				...
				<li>
					<span class='suite-started-time'style="color:white !important">${.now?datetime?string(dateTimeFormat)}</span>
				</li>
				...
				<div class='time-totals'>
					...
					<div class='col l2 m4 s6 suite-start-time'>
						<div class='card accent green-accent' >  
							...
							<span class='panel-lead suite-started-time' style="color:white !important">${report.startedTime?datetime?string(dateTimeFormat)}</span> 
						</div> 
					</div>
					<div class='col l2 m4 s6 suite-end-time' style="color:white !important">
						<div class='card accent pink-accent' > 
							<span class='panel-name' >${resourceBundle.getString("dashboard.panel.name.end")}</span> 
							<span class='panel-lead suite-ended-time' style="color:white !important">${.now?datetime?string(dateTimeFormat)}</span> 
						</div> 
					</div>
				</div>
				<div class='charts'>
							...
							<div class='wrapper'>
								<ul id='test-collection' class='test-collection'>
									<#list report.testList as extentTest>
										...
										<div class='test-head'>
												<span class='category-assigned' style="font-weight: bold;font-size: 16px;"><#list test.categoryList as category> ${category.name}</#list></span>
										</div>
										...
										<li class='collection-item test displayed active ${test.status} <#if test.nodeList?size != 0>hasChildren</#if>' extentid='${test.id?string}'>
											<div class='test-head'>
											<a href="#">
												<span class='test-name' style="text-decoration: underline;font-weight: bold;">${test.name} <#if test.internalWarning??><i class='tooltipped mdi-alert-error' data-position='top' data-delay='50' data-tooltip='${test.internalWarning}'></i></#if></span>
												</a>
